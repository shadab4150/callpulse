...
import json
from fetcher import EarningsCallFetcher
import openai
...
class TranscriptAnalyzer:
    def __init__(self, openai_api_key=openai_api_key, mongo_uri=mongodb_uri, api_ninja_key=api_ninja_key):
        """
        ...
        self.fetcher = EarningsCallFetcher(api_key=api_ninja_key)
    ...
    def analyze_transcripts(self, ticker, prompt_type, n_jobs=4):
        """
        ...
        """
        ticker = ticker.upper()
        results = self.fetcher.get_last_n_quarters(ticker, 4)
        keys_to_keep = ['date', 'transcript_split', 'ticker', 'year', 'quarter']
        transcript_data_list = [{k: d[k] for k in keys_to_keep if k in d} for d in results]
