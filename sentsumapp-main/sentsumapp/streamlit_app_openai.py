...
from utils.analyzer import TranscriptAnalyzer
...
if openai_api_key:
    ...
    # Initialize the transcript analyzer with OpenAI client and MongoDB connection
    analyzer = TranscriptAnalyzer(openai_client, mongodb_uri)
else:
    openai_client = None
    analyzer = None
    st.warning("OPENAI_API_KEY not found in .env file. Please add it to use the analysis functionality.")
...
with col1:
            ...
            with st.spinner("Fetching transcripts..."):
                try:
                    ...
                    if results:
                        ...
                        st.success(f"Successfully fetched data for {ticker} (last 4 quarters)")
                    else:
    ...
    if st.button("Step 2: Summarize Data"):
        if analyzer is None:
            st.error("OpenAI API key not available. Please add it to the .env file.")
            ...
            with st.spinner("Summarizing transcripts in parallel..."):
                try:
                    ...
                    st.success("Successfully summarized all transcripts")
                except Exception as e:
        ...
        if analyzer is None:
            ...
            with st.spinner("Extracting topics in parallel..."):
                try:
                    ...
                    st.success("Successfully extracted topics from all transcripts")
                except Exception as e:
        ...
        if analyzer is None:
            ...
            with st.spinner("Calculating sentiment in parallel..."):
                try:
                    ...
                    st.success("Successfully calculated sentiment for all transcripts")
                except Exception as e:
                ...
                for data in sorted_data:
                    ...
                    with st.expander(expander_title):
                        ...
                        if transcript_id in st.session_state.summaries:
                            st.write(start_text+st.session_state.summaries[transcript_id])
                        else:
                    ...
                    with st.expander(expander_title):
                        if transcript_id in st.session_state.topics:
                            ...
                            if company_name!=None:
                                ...
                                start_text = f"### Company Name : {ticker} \n\n"
                            st.write(start_text+st.session_state.topics[transcript_id])
                        else:
